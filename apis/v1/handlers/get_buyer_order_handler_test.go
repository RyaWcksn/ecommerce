...
func TestHandlerImpl_GetBuyerOrdersHandler(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantMock: func() {
				s.EXPECT().GetBuyerOrderList(gomock.Any()).Return(
					&[]entities.Order{
						{
							Id:                  1,
							Buyer:               2,
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := h.GetBuyerOrdersHandler(tt.args.w, tt.args.r); (err != nil) != tt.wantErr {
				t.Errorf("HandlerImpl.GetBuyerOrdersHandler() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
