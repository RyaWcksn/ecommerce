...
func TestOrderImpl_UpdateOrder(t *testing.T) {
	type fields struct {
		DB  *sql.DB
		log logger.ILogger
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gotOrder, err := o.UpdateOrder(tt.args.ctx, tt.args.id)
			if (err != nil) != tt.wantErr {
				t.Errorf("OrderImpl.UpdateOrder() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(gotOrder, tt.wantOrder) {
				t.Errorf("OrderImpl.UpdateOrder() = %v, want %v", gotOrder, tt.wantOrder)
			}
		})
	}
}

func TestOrderImpl_BuyerViewOrderList(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("Failed to create mock: %v", err)
	}
	...
	query := regexp.QuoteMeta(GetByBuyerId)
	rows := sqlmock.NewRows([]string{"id", "buyer", "seller", "delivery_source", "delivery_destination", "items", "quantity", "price", "total_price", "status"}).
		AddRow(1, 2, 1, "Source", "Destination", "Items", 4, "180000", "180000", 0)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			o := NewOrderImpl(db, log)
			gotOrder, err := o.BuyerViewOrderList(tt.args.ctx, tt.args.id)
			if (err != nil) != tt.wantErr {
				t.Errorf("OrderImpl.BuyerViewOrderList() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(gotOrder, tt.wantOrder) {
				t.Errorf("OrderImpl.BuyerViewOrderList() = %v, want %v", gotOrder, tt.wantOrder)
			}
		})
	}
}
